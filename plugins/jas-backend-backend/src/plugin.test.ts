...
import { jasBackendPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [jasBackendPlugin],
    });

    await request(server).get('/api/jas-backend/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/jas-backend/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/jas-backend/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/jas-backend/todos/${createdTodoItem.id}`)
